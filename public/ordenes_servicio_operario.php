<?php
// filepath: /opt/lampp/htdocs/Proyectos/Emar/public/ordenes_servicio_operario.php
...
// Procesar actualización de estado y observaciones con registro en historial
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['orden_id'])) {
    ...
    $stmt_old = $mysqli->prepare("SELECT estado, observaciones FROM ordenes_servicio WHERE id=?");
    $stmt_old->bind_param("i", $orden_id);
    $stmt_old->execute();
    $stmt_old->bind_result($old_estado, $old_obs);
    $stmt_old->fetch();
    $stmt_old->close();

    // Actualizar orden
    $stmt = $mysqli->prepare("UPDATE ordenes_servicio SET estado=?, observaciones=? WHERE id=?");
    $stmt->bind_param("ssi", $nuevo_estado, $observaciones, $orden_id);
    $stmt->execute();
    $stmt->close();
...
$sql = "SELECT o.id, o.tipo, o.descripcion, o.fecha_solicitud, o.estado, o.observaciones, u.nombre AS usuario_nombre, u.email AS usuario_email
...
<head>
    <meta charset="UTF-8">
    <title>Órdenes de Servicio Asignadas</title>
...
<body>
    <div class="container">
        <h1>Órdenes de Servicio Asignadas</h1>
        ...
        <table>
            ...
            <tr>
                <td class="no-data" data-label="Sin datos" colspan="9">No tienes órdenes asignadas.</td>
            </tr>
            <?php endif; ?>
        </table>
        <br>
        <a href="menu_operario.php">Volver al menú</a>
    </div>
</body>
</html>