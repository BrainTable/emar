<?php
// filepath: /opt/lampp/htdocs/Proyectos/Emar/public/eliminar_medidor.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $csrf_token) {
        ...
        $accion = "Eliminación";
        $descripcion = "Medidor eliminado por el usuario ID " . $_SESSION['usuario_id'];
        $stmt_hist = $mysqli->prepare("INSERT INTO historial_medidor (medidor_id, usuario_id, accion, descripcion) VALUES (?, ?, ?, ?)");
        $stmt_hist->bind_param("iiss", $id, $_SESSION['usuario_id'], $accion, $descripcion);
        $stmt_hist->execute();
        $stmt_hist->close();

        // Eliminar el medidor
        $stmt = $mysqli->prepare("DELETE FROM medidores WHERE id=?");
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) {
            $mensaje = "Medidor eliminado correctamente.";
            $mensaje_tipo = "exito";
        } else {
        ...
        header("Location: medidores_admin.php?msg=" . urlencode($mensaje));
        exit;
    }
}
?>
    ...
    <div class="container">
        ...
        <?php else: ?>
            ...
            <form method="post">
                ...
                <a href="medidores_admin.php"><button type="button">Cancelar</button></a>
            </form>
        <?php endif; ?>
    </div>
</body>
